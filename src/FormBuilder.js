...
const FormBuilder = () => {
  ...
  const handleAddField = (type) => {
    const newField = {
      ...
      placeholder: "",
      options:
        type === "checkbox" || type === "radio" ? ["Option 1"] : undefined,
    };
  ...
  const handleSortUpdate = (sortedIDs) => {
    const sortedFields = sortedIDs.map((id) =>
      formFields.find((field) => field.id === id)
    );
      ...
      <SortableWrapper onUpdate={handleSortUpdate}>
        {formFields.map((field, index) => (
          ...
          >
            ...
            <button className="mx-2" onClick={() => handleEditField(index)}>
              Edit
            </button>
